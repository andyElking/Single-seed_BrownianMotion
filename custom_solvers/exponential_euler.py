...
class _ExpEulerCoeffs(AbstractCoeffs):
    ...
    def __init__(self, beta_half, a_half, beta1, a1, b1, aa, chh):
...
class ExpEuler(AbstractFosterLangevinSRK[_ExpEulerCoeffs, None]):
    r"""The Exponential Euler method.
    This is an order 1 solver for the Underdamped Langevin Diffusion.
    ...
    def _tay_coeffs_single(self, c: UnderdampedLangevinLeaf) -> _ExpEulerCoeffs:
        del self
        # c is a leaf of gamma
    ...
    def _compute_step(
        ...
        fx0 = f(x0, args)
        fx0_uh = (fx0**ω * u**ω * h).ω
        v1 = (
            coeffs.beta1**ω * v0**ω
            - coeffs.beta_half**ω * fx0_uh**ω
            + rho**ω * (coeffs.aa**ω * w**ω - gamma**ω * coeffs.chh**ω * hh**ω)
        ).ω
        x1 = (
            x0**ω
            + coeffs.a1**ω * v0**ω
            - coeffs.a_half**ω * fx0_uh**ω
